    ...
    fluidRow(
      ...
      column(width = 12,
             box(title = "SNP calling efficiency - without filters",
                 width = NULL, solidHeader = TRUE, collapsible = FALSE,
                 status="primary",
      ...
      column(width = 6,
             box(width = 6, solidHeader = T,
                 fluidPage(
                   checkboxGroupInput(ns("SNPCall"),
                                      label = p("SNP calling method"),
...
#'
#' @noRd
mod_simu_SNPCalling_efficiency_server <-
  function(input, output, session, datas_simu){
    ns <- session$ns
    observe({
    ...
    button <- eventReactive(input$go, {
      withProgress(message = 'Building graphic', value = 0, {
        ...
        idx <- which(grepl(paste(input$depth, collapse = "|"),
                           names_files) &
                       grepl(paste(input$SNPCall,collapse = "|"),
                             names_files) &
